...
class WeeklyCalendar:
    ...
    def __init__(self):
        ...
        # ensure keys exist
        """#chatgpt ha escluso queste righe
          for d in WeeklyCalendar.DAYS:
            self.bookings.setdefault(d, {})
...
# -------------------
class User:
    def __init__(self, user_id: int, username: str, initial_minutes: int):
    ...
    @classmethod
    def from_dict(cls, d):
        u = cls(d["user_id"], d.get("username", ""), d.get("initial_minutes", 0))
    ...
    def reset_daily(self, new_initial: Optional[int] = None, reset_remaining: bool = True):
    ...
    async def _sensor_loop(self):
        while True:
            try:
                ...
                logger.warning("MQTT connection failed for %s: %s. Reconnecting in 10s...", self.name, e)
...
class SystemManager:
    def __init__(self, config: dict, calendar: WeeklyCalendar):
        ...
        if not self.users:
            ...
            placeholders = [("Manoel", default_minutes), ("user2", default_minutes), ("user3", default_minutes), ("user4", default_minutes), ("user5", default_minutes)]
            for i, (uname, mins) in enumerate(placeholders, start=1):
                uid = 100000 + i
                self.users[uid] = User(uid, uname, mins)
            self.persist_users()

        # attach Manoel to first plug by default if exists
        manoel = None
        for u in self.users.values():
            if u.username and u.username.lower() == "manoel":
                manoel = u
                break
        if manoel:
            first_plug = next(iter(self.plugs.values()))
            manoel.attach_plug(first_plug)
            logger.info("Manoel attached to plug %s by default", first_plug.name)
            self.persist_users()
    ...
    def get_user_by_telegram(self, tg_user) -> User:
        ...
        default_minutes = int(self.config.get("default_daily_minutes", DEFAULT_CONFIG["default_daily_minutes"]))
        user = User(tg_user.id, tg_user.username or tg_user.first_name or str(tg_user.id), default_minutes)
    ...
    async def _background_loop(self, bot_application: Application):
        ...
        while True:
            try:
                ...
                for plug in self.plugs.values():
                    ...
                    if user and power > self.power_threshold:
                        ...
                        if check_user_time(self.calendar, user, datetime.now()):
                            ...
                            if consumed > 0:
                                self.persist_users()
                                """ msg = (f"⏱ {user.username}: using {plug.name}. "
                                       f"Consumed {consumed} min. Remaining: {user.remaining_minutes} min.") 
                                await broadcast_message(bot_application, msg)"""
                            # if user exhausted
                            if user.remaining_minutes <= 0 or (user.username == "Manoel" and user.remaining_minutes - user.error_minutes<=0):
                ...
                if error_in_any_plug:
                    self.users[100001].error_minutes += interval  # example: increment error minutes for user with id 100001
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Remaining minutes: {user.remaining_minutes} / {user.initial_minutes}.\n"
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for plug in manager.plugs.values():
        ...
        lines.append(
            f"  • {plug.name}: power={plug.last_power:.1f}W, user={owner}, "
            f"errors={plug.error_minutes} min, state={error_status}, status={active_status}"
        )
    lines.append("")
    for u in manager.users.values():
        lines.append(
            f"  • @{u.username} ({u.user_id}): remaining={u.remaining_minutes} min, error={u.error_minutes} min"
        )
    lines.append("")    
    lines.append(f"Uptime: {uptime_str} Commands: /startplug <plugname>, /stopplug <plugname>, /status")
    await update.message.reply_text("\n".join(lines))
...
async def stopplug_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        if user.active_plug:
            ...
            user.detach_plug()
            # reassign Manoel
            if pname == 'plug_salotto' and user.username.lower() != "manoel":
                manoel=None
                for u in manager.users.values():
                    if u.username and u.username.lower() == "manoel":
                        manoel = u
    ...
    if plug.user and plug.user.user_id == user.user_id:
        ...
        # reassign Manoel
        if plugname == 'plug_salotto' and user.username.lower() != "manoel":
            manoel=None
            for u in manager.users.values():
                if u.username and u.username.lower() == "manoel":
                    manoel = u
...
async def set_daily_minutes_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_user:
        await update.message.reply_text("Target user not found")
        return
    target_user.reset_daily(minutes, reset_remaining=False)
    manager.persist_users()
    await update.message.reply_text(f"{target_user.username} daily minutes set to {minutes}. Remaining reset.")
...
#   # admin-only: my_bookings (shows bookings of admin or of specified user)
...
async def admin_onoff_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_authorized(update.effective_user.id):
        await update.message.reply_text("You are not authorized to use this command.")
        return
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/listplugs - admin only\n"
        "/addminutes <user_id|@username> <minutes> - admin only\n"
        "/setDailyMinutes <user_id|@username> <minutes> - admin only\n"
        "/calendar - admin only (view week)\n"
        "/book <user_id|@username> - admin only (open booking UI)\n"
        "/my_bookings [user_id|@username] - admin only\n"
        "/activate <activate|deactivate> <plugname> - admin only\n"
    )
    ...
    try:
        ...
        for plug in manager.plugs.values():
            if plug.error_minutes > 0:
                plugs_with_errors.append(f"  • {plug.name}: {plug.error_minutes:.1f} minutes in error")
        ...
        for user in manager.users.values():
            consumed = user.initial_minutes - user.remaining_minutes
            if consumed > 0:
        ...
        # PERFORM DAILY RESET
        default_minutes = int(manager.config.get("default_daily_minutes", DEFAULT_CONFIG["default_daily_minutes"]))

        # Reset user minutes
        users_reset = []
        for user in manager.users.values():
            old_remaining = user.remaining_minutes
            user.reset_daily()  # Reset to initial_minutes
            users_reset.append(f"  • @{user.username}: {old_remaining} → {user.remaining_minutes} minutes")
        ...
        message_lines.extend([
            "",
            f"🎯 **Ready for new day!** All users have {default_minutes} minutes available.",
...
async def send_half_hour_stats(app: Application):
    ...
    try:
        ...
        message_lines = [
            f"📊 **Half-Hour Stats Report**,  Uptime: {uptime_str}",
        ...
        for plug in manager.plugs.values():
            ...
            message_lines.append(
                f"  • {plug.name}: {plug.last_power:.1f}W, {user_info}, {status_info}{last_seen_info}"
            )
        ...
        for user in manager.users.values():
            if user.remaining_minutes > 0:
                users_with_minutes.append(f"@{user.username}: {user.remaining_minutes}min")

            if user.active_plug:
                users_with_plugs.append(f"@{user.username} → {user.active_plug.name}")

        message_lines.append(f"  • Total users: {total_users}")

        if users_with_minutes:
            message_lines.append(f"  • Users with minutes:")
            for user_info in users_with_minutes[:5]:  # Show max 5 users
                message_lines.append(f"    - {user_info}")
            if len(users_with_minutes) > 5:
                message_lines.append(f"    - ... and {len(users_with_minutes) - 5} more")
        else:
            message_lines.append(f"  • No users have remaining minutes")
...
def main():
    ...
    application.add_handler(CommandHandler("plug", admin_onoff_command))
